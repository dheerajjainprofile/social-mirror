...
// NOTE: This file is in src/tests/components/ and is excluded from the default
// vitest run (vitest.config.ts excludes src/tests/components/**).
// Run manually: npx vitest run --include src/tests/components/roundStartFixes.test.ts
// All tests here are pure logic (no renderHook/DOM) and can be safely moved to
// src/tests/ if the vitest config is updated to include components.
