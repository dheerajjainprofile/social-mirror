...
export interface BadgeShareContext {
  ...
  roomCode?: string
  totalRounds?: number
  appUrl?: string          // e.g. https://hunch.vercel.app
}

function stripInvalidFilenameChars(s: string): string {
  return s.replace(/[^a-zA-Z0-9-_]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 40) || 'Hunch'
}

// Filename convention for ALL artifacts:
//   Hunch-<kind>-<Person>-<Detail>.png
...
export function badgeFileName(playerName: string, badge: PlayerBadge): string {
  ...
  return `Hunch-Badge-${person}-${name}.png`
}
...
export function sessionStoryFileName(hostName: string, roomCode: string): string {
  ...
  return `Hunch-Game-${person}-${room}-${date}.png`
}
...
function badgeHook(ctx: BadgeShareContext): string {
  ...
  switch (badge.name) {
    case 'The Baba Vanga':
      return `${playerName} just got called 🔮 The Baba Vanga in Hunch.\nPredicted strangers' answers DOWN TO THE NUMBER.${rank ? ` Finished ${rank}.` : ''}\nThink you can read minds too?`
    case 'The Virat Kohli':
      return `${playerName} went full 🔥 Virat Kohli in Hunch — won three rounds in a row like every point was personal.${rank ? ` Finished ${rank}.` : ''}\nTry to beat this streak.`
    case 'The MS Dhoni':
      return `🏏 ${playerName} = MS Dhoni in Hunch. Cool head, closest guess, finished it every single time.${rank ? ` Came in ${rank}.` : ''}\nThink you're calmer under pressure?`
    case 'The Salman Khan':
      return `${playerName} earned the 🕶️ Salman Khan badge in Hunch — broke every rule of the game and somehow still won. Bhai things.\nCome try to beat it.`
    case 'The Aamir Khan':
      return `${playerName} is officially 🎬 The Aamir Khan of Hunch — took forever, got every answer right. Perfectionist certified.${rank ? ` Finished ${rank}.` : ''}`
    case 'The Mogambo':
      return `Nobody could crack ${playerName} in Hunch tonight. 🕵️ The Mogambo badge earned.\nMogambo khush hua. Think YOU can guess what they're thinking? [link]`
    case 'The SRK':
      return `🌟 ${playerName} was THE main character in Hunch tonight — the whole room was guessing about them. Obviously.\nHost your own and see who becomes the SRK of your group.`
    case 'The Arnab Goswami':
      return `🎙️ The nation demanded an answer from ${playerName} in Hunch. They answered. Loudly. Wrongly. But FAST.\nThink you're faster?`
    case 'The Ambani':
      return `💰 ${playerName} guessed in crores while everyone else guessed in normal numbers. The Ambani of Hunch has spoken.\nChallenge their vibe.`
    case 'The Hardik Pandya':
      return `⚡ No plan. Just vibes. Somehow right.\n${playerName} just earned The Hardik Pandya badge in Hunch.${rank ? ` Finished ${rank}.` : ''}\nCan YOUR vibes match?`
    case 'The Gabbar Singh':
      return `😬 "Kitne aadmi the?" ${playerName} was still completely wrong.\nThe Gabbar Singh of Hunch has been crowned. Come laugh with them.`
    case 'The Devdas':
      return `👻 ${playerName} passed so many rounds in Hunch they got called The Devdas. Present. Suffering. Uninvolved.\nTag them. They deserve it.`
    case 'The Babu Bhaiya':
      return `🤷 Haan... nahi... pata nahi.\n${playerName} just earned the 🤷 Babu Bhaiya badge in Hunch — wrong every single time. Iconic.\nCome be wrong with them.`
    default:
      return `${playerName} just earned ${badge.emoji} ${badge.name} in Hunch.${exact ? ' Got an answer EXACTLY right.' : ''}${rank ? ` Finished ${rank}.` : ''}`
  }
}

export function badgeShareText(ctx: BadgeShareContext): string {
  const hook = badgeHook(ctx)
  const link = ctx.appUrl ?? 'https://hunch.vercel.app'
  return `${hook}\n\n🎯 Play Hunch: ${link}`
}
...
export function sessionStoryShareText(ctx: SessionShareContext): string {
  ...
  const winnerLine = ctx.winnerName
    ...
    : ''
  return `🎯 ${ctx.hostName} just ran a Hunch night — ${ctx.playerCount} players, ${ctx.roundsPlayed} rounds of mind-reading chaos.\n${winnerLine}\n\nHost your own group: ${link}`
}

export function sessionStoryShareTitle(ctx: SessionShareContext): string {
  return `${ctx.hostName}'s Hunch game · Room ${ctx.roomCode}`
}
